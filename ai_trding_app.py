...
# ================= PREP DATA =================
scaler = MinMaxScaler()
scaled = scaler.fit_transform(close_array.reshape(-1, 1))
...
pred_price = float(scaler.inverse_transform(pred).flatten()[0])

# 🔥 SAFE current price extraction (NO Series ERROR EVER)
current_price = float(close_array[-1])

# ================= SIGNAL ENGINE =================
diff = float(pred_price - current_price)
...
final_score = (0.6 * lstm_signal) + (0.4 * sent_signal)
